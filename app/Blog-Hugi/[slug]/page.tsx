...
export async function generateStaticParams() {
  ...
  const allPosts = [
    ...Object.entries(singleRaw).map(([key, p]: any) => ({ slug: p.slug ?? key })),
    ...Object.entries(multiRaw).map(([key, p]: any) => ({ slug: p.slug ?? key })),
  ];
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const title = post.title || "아파트 도배시공 – 따뜻하고 아늑한 집 만드는 과정";
  const description =
    post.summary || "아파트·원룸·빌라 도배, LH도배 장판 시공으로 빠르고 깔끔하게 이렇게 해보았어요";
  const image = post.thumbnailUrl ? `${post.thumbnailUrl}?w=1200&h=630` : "/default-og-1200x630.jpg";
...
export default async function BlogHugiPostSSR({ params }: PageProps) {
  ...
  const ogImage = post.thumbnailUrl ? `${post.thumbnailUrl}?w=1200&h=630` : "/default-og-1200x630.jpg";
    ...
    <>
      ...
      <Script id="json-ld" type="application/ld+json">
        {JSON.stringify({
          ...
          "@type": "Article",
          headline: post.title?.trim() || "아파트 도배시공 – 따뜻하고 아늑한 집 만드는 과정",
          description: post.summary || "아파트·원룸·빌라 도배, LH도배 장판 시공으로 빠르고 깔끔하게 이렇게 해보았어요",
          image: [encodeURI(ogImage)],
          author: { "@type": "Organization", name: "월스타일" },
