...
const handleSaveGroup = async () => {
  ...
  try {
    ...
    for (let i = 0; i < groupPosts.length; i++) {
      ...
      await set(dbRef(db, `multimage/${postId}`), {
        title: post.title,
  ...
  const deletePost = async (postId: string, storagePath: string) => {
    ...
    try {
      ...
      await remove(dbRef(db, `multimage/${postId}`));
      alert("삭제 완료");
    } catch (e) {
      console.error(e);
      alert("삭제 오류");
  ...
  const deleteGroup = async (groupId: string) => {
    ...
    try {
      ...
      for (const post of group) {
        ...
        await remove(dbRef(db, `multimage/${post.id}`));
      }
  ...
  const updatePostImage = async (
    ...
    try {
      ...
      await update(dbRef(db, `multimage/${postId}`), {
        imageUrl: image.webp.url,
        thumbnailUrl: image.webp.url,
         ogUrl: image.jpg.url, 
        storagePath: image.webp.path,
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      await update(dbRef(db, `multimage/${editing.id}`), {
        title: editing.title,
  ...
  /* -------- 데이터 실시간 불러오기 -------- */
  useEffect(() => {
    const postsRef = dbRef(db, "multimage");
